...
function Navbar(){
  ...
  const viewCoinInfo = (ID)=>{
    ...
    return(
      <nav className="flex justify-between w-full h-16 items-center shadow-xl z-50 bg-[#6E6E6E] fixed top-0">
        ...
        <div className={`w-6/12 h-[10rem] bg-[#999999] flex flex-col absolute top-[82%] right-[16%] rounded-sm shadow-3xl transition-all duration-300 ease-in-out ${openMenu ? "block" : "hidden"} items-center justify-center text-white`}>
        ...
        <div className={`absolute top-[29%] right-24 sm:top-[25%] sm:right-[10%]`}>
          <img width="31px" height="31px" className="text-2xl" src="./icons8-search-30.svg" onClick={()=>{
            setOpenSearch(!openSearch);
            if(openMenu){
              setOpenMenu(!openMenu);
            }
        ...
        <div className={`w-8/12 h-auto bg-[#999999] flex flex-col absolute top-[82%] right-[16%] rounded-sm shadow-3xl transition-all duration-300 ease-in-out ${openSearch ? "block" : "hidden"}`}>
        ...
        <ul className={`w-full z-50 fixed top-0 h-screen md:hidden bg-[#303030] text-white text-center text-2xl ${isNavOpen? "translate-x-0" : "-translate-x-full"} transition-all duration-300`}>
          <li className="absolute top-4 left-3">
            <Link href="/">
              <a className="text-3xl text-[#BCFD4C]">
                CoinParadise
              </a>
            </Link>
          </li>
          ...
          <li className="mb-8" onClick={toggleNav}><Link href="/submit"><a className={`hover:text-neutral-500 rounded-md hover:transition-all duration-300 ${path == "/submit" ? "shadow-[0_15px_4px_-15px_#BCFD4C]" : ""}`}>Submit</a></Link></li>
