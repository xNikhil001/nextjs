  ...
  const submitPage = ()=>{
    ...
    return(
      <nav className="flex justify-between w-full h-16 items-center shadow-xl z-50 bg-[#232931] fixed top-0">
        <div className="absolute top-3 left-5"><Link href="/"><a><img src="/logo.webp" width="35px" height="35px" /></a></Link></div>
        <div></div>
        ...
        <div className={`absolute top-[29%] right-24 sm:top-[25%] sm:right-[10%]`}>
          <img width="31px" 
          height="31px" className="text-2xl" src="/icons8-search-30.svg" onClick={()=>{
            setOpenSearch(!openSearch);
            if(openMenu){
              setOpenMenu(!openMenu);
            }
        ...
        <ul className={`w-full z-50 fixed top-0 h-screen md:hidden bg-[#232931] text-white text-center text-2xl ${isNavOpen? "translate-x-0" : "-translate-x-full"} transition-all duration-300`}>
          <li className="absolute top-3 left-5">
            <img src="/logo.webp" width="35px" height="35px" />
          </li>
          <li className="absolute top-4 right-5">
            <span className="text-3xl text-[#4ECCA3]" onClick={toggleNav}>&#x2715;</span>
          </li>
          <li className="mt-[40%] mb-8" onClick={toggleNav}><Link href="/"><a className={`hover:text-neutral-500 rounded-md hover:transition-all duration-300 ${path == "/" ? "shadow-[0_15px_4px_-15px_#4ECCA3]" : ""}`}>Home</a></Link></li>
          <li className="mb-8" onClick={toggleNav}><a onClick={submitPage} className={`hover:text-neutral-500 rounded-md hover:transition-all duration-300 ${path == "/submit" ? "shadow-[0_15px_4px_-15px_#4ECCA3]" : ""}`}>Submit</a></li>
          <li className="mb-8" onClick={toggleNav}><Link href="/promote"><a className={`hover:text-neutral-500 rounded-md hover:transition-all duration-300 ${path == "/promote" ? "shadow-[0_15px_4px_-15px_#4ECCA3]" : ""}`}>Promote</a></Link></li>
        </ul>
      </nav>
    )
}
